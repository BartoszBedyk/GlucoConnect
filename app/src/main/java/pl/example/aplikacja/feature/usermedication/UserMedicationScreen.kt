            ...
            Box(Modifier.fillMaxSize()) {
                ...
                Column(Modifier.fillMaxSize()) {
                    LazyColumn {
                        items(medications.value) { medication ->
                            UserMedicationSwapItem(
                                medication,
                                modifier = Modifier,
                                {},
                                {},
                                {
                                        umId, medicationId  -> navController?.navigate("medication_result/$umId/$medicationId")
                                })
                            HorizontalDivider(
                                thickness = 1.dp,
                ...
                Column(
                    ...
                    FloatingActionButton(
                        ...
                        shape = Shapes().medium,
                        modifier = Modifier
                            .padding(bottom = 16.dp, top = 4.dp, start = 16.dp, end = 16.dp),
                        elevation = FloatingActionButtonDefaults.elevation(4.dp)
                    ) {
...
@Composable
fun MedicationItem(medication: UserMedicationResult, onItemClick: (String, String) -> Unit) {
    Row(
        modifier = Modifier
            .padding(16.dp)
            .fillMaxWidth()
            .clickable {
                onItemClick(medication.id.toString(), medication.medicationId.toString())
            }
    ) {
        Log.d("MedicationItem", "medication: $medication")
