...
fun GlucoseResultDB.toResearchResult(): ResearchResult = this.let {
    ResearchResult(
        id = it.id,
...
fun ResearchResult.toGlucoseResultDB(): GlucoseResultDB = this.let {
    GlucoseResultDB(
        id = it.id,
        glucoseConcentration = it.glucoseConcentration,
...
private fun GlucoseUnitTypeDB.toGlucoseUnitType(): GlucoseUnitType = when (this) {
...
private fun GlucoseUnitType.toGlucoseUnitTypeDB(): GlucoseUnitTypeDB = when (this) {
    GlucoseUnitType.MG_PER_DL -> GlucoseUnitTypeDB.MG_PER_DL
    GlucoseUnitType.MMOL_PER_L -> GlucoseUnitTypeDB.MMOL_PER_L
    else -> throw IllegalArgumentException("Unknown GlucoseUnitTypeDB: $this")
}

fun List<GlucoseResultDB>.toResearchResultList(): List<ResearchResult> = this.map { it.toResearchResult() }

fun List<ResearchResult>.toGlucoseResultDBList(): List<GlucoseResultDB> = this.map { it.toGlucoseResultDB() }

fun ResearchResultCreate.toGlucoseResultDB(userId: String): GlucoseResultDB = this.let {
    GlucoseResultDB(
        id = UUID.randomUUID(),
        glucoseConcentration = it.glucoseConcentration,
